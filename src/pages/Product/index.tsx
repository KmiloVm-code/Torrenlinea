...
const ProductPage = () => {
  ...
  const { productDetail, loadDetails } = useContext(ProductDetailContext);
      ...
      <section className="relative w-full p-12 md:py-24 lg:p-32 2xl:p-56">
        <div className="container flex flex-col md:flex-row items-start px-4 md:px-6">
          ...
          <div className="flex flex-col md:w-1/2 md:ml-6 mt-4 md:mt-0 gap-8 tracking-tighter">
            ...
            <p className="text-2xl font-semibold text-gray-800" >{formatPrice(parseInt(productDetail?.price ?? '0'))}</p>
            <div className="text-gray-800 font-normal sm:text-lg" dangerouslySetInnerHTML={{ __html: productDetail?.short_description ?? '' }} />
